...
SCHEDULER_CHOICES = ['step', 'cosine', 'plateau', 'none']  # 'none' means no scheduler
...
def run_trial(trial_id, cfg_dict, *, log_tb=True):
    ...
    scheduler = None
    if cfg_dict.get("scheduler", "none") != "none":
        if cfg_dict["scheduler"] == "step":
            scheduler = StepLR(optimizer, 
                              step_size=cfg_dict.get("scheduler_step_size", 10),
                              gamma=cfg_dict.get("scheduler_gamma", 0.5))
        elif cfg_dict["scheduler"] == "cosine":
            scheduler = CosineAnnealingLR(optimizer,
                                         T_max=cfg_dict.get("scheduler_t_max", MAX_EPOCHS),
                                         eta_min=0)
        elif cfg_dict["scheduler"] == "plateau":
            scheduler = ReduceLROnPlateau(optimizer,
                                         mode='max',  # monitor validation accuracy
                                         patience=cfg_dict.get("scheduler_patience", 5),
                                         factor=cfg_dict.get("scheduler_factor", 0.5))
    ...
    for epoch in range(1, MAX_EPOCHS+1):
        ...
        if scheduler is not None:
            if cfg_dict["scheduler"] == "plateau":
                scheduler.step(val_acc)  # ReduceLROnPlateau uses validation metric
            else:
                scheduler.step()  # StepLR and CosineAnnealingLR step every epoch
        
        if writer and scheduler is not None:
            writer.add_scalar("LR", current_lr(optimizer), global_step)
...
def random_search(n_trials=N_TRIALS, log_tb=True):
    ...
    all_cfgs = list(itertools.product(D_MODEL_CHOICES, LAYER_CHOICES, HEAD_CHOICES,
                                  ATTN_ONLY_CHOICES, WD_CHOICES, LR, SCHEDULER_CHOICES,
                                  SCHEDULER_STEP_SIZE, SCHEDULER_GAMMA, SCHEDULER_T_MAX,
                                  SCHEDULER_PATIENCE, SCHEDULER_FACTOR))
    results = []
    print(all_cfgs)
    for t, (d_model, n_layers, n_heads, attn_only, weight_decay, lr, scheduler, 
            step_size, gamma, t_max, patience, factor) in enumerate(all_cfgs):
        cfg_dict = {
            "d_model": d_model,
            "n_layers": n_layers,
            "n_heads":  n_heads,
    ...
    print("\nTop configs:")
    print(df.head(10)[["best_val_acc","best_epoch","n_layers","n_heads","attn_only","lr","weight_decay","scheduler","run_name"]])
    print(f"\nSaved: {out_csv}")
    return df
